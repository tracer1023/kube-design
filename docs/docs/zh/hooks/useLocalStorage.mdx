...
description: '使用 localStorage 值作为 react 状态，通过打开的浏览器 tab 页同步状态'
order: 1
---

## 使用方法

useLocalStorage 允许您使用 localStorage 中的值作为 react 状态。 useLocalStorage 的工作方式与 useState 完全相同，但也会将值写入本地存储。
...
useLocalStorage 订阅存储事件。当一个 tab 页中的状态发生变化时，它会自动更新所有其他打开的浏览器 tab 页中的值。
...
您可以指定与 useState hook 中相同的值类型，类型必须扩展字符串。
